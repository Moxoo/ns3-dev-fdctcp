...
NS_LOG_COMPONENT_DEFINE ("Outcast");
...
void
ThroughputPerSecond(Ptr<PacketSink> sink1Apps,std::string filePlotThrough, uint32_t f_num)
{
	...
	Simulator::Schedule (Seconds(interval), &ThroughputPerSecond, sink1Apps, filePlotThrough, f_num);
	std::ofstream fPlotThroughput (filePlotThrough.c_str (), std::ios::out | std::ios::app);
	fPlotThroughput << "1:" << f_num << " " << currentPeriodRecvBytes * 8 / (interval * 1000000) << " " << (972-currentPeriodRecvBytes * 8 / (interval * 1000000))/f_num << std::endl;
//	fPlotThroughput << "1:" << f_num << " " << Simulator::Now().GetSeconds() << " " << currentPeriodRecvBytes * 8 / (interval * 1000000) << std::endl;
}

int main (int argc, char *argv[])
{
  // Configure information
  std::string prefix_file_name = "Outcast";
  uint32_t sendNum = 2;
  uint32_t f_num = 1;
  std::string transport_port = "TcpDctcp";
  std::string queue_disc_type = "RedQueueDisc";//only for dctcp

  std::string queue_limit = "100p";
  double K = 30;
...
//  Ptr<UniformRandomVariable> random = CreateObject<UniformRandomVariable> ();
  ...
  cmd.AddValue ("f_num","Number of left and right side leaf nodes", f_num);
  ...
  // To enable logging
  LogComponentEnable("Outcast", LOG_LEVEL_INFO);
...
//  LogComponentEnable("TcpDctcp", LOG_LEVEL_INFO);
  ...
  for(uint16_t i=0; i<senders.GetN(); i++)
  {
	...
	//	  sourceApp.Stop(Seconds(stop_time));
	  ...
	  {
		  std::vector<ApplicationContainer> sourceApp_V;
		  for(uint32_t j=0; j<f_num; j++){
			  BulkSendHelper ftp("ns3::TcpSocketFactory", InetSocketAddress(sink_interfaces.GetAddress(0,0),port+1));
			  ftp.SetAttribute("SendSize", UintegerValue(TCP_SEGMENT));
			  ftp.SetAttribute("MaxBytes", UintegerValue(data_mbytes));
			  ApplicationContainer sourceApp = ftp.Install(senders.Get(i));
			  sourceApp.Start(Seconds(start_time));
			  sourceApp_V.push_back(sourceApp);
		  }
	  }
  ...
  PacketSinkHelper sinkHelper1("ns3::TcpSocketFactory", InetSocketAddress (Ipv4Address::GetAny(), port+1));
...
//	  Simulator::Schedule (Seconds (start_time + 0.000001), &TraceRtt, "rtt.data");
	  ...
	  // Get throughput
	  filePlotThroughput << "outcast-" << id << "-throughput.plotme";
//	  remove (filePlotThroughput.str ().c_str());
	  Simulator::ScheduleNow (&ThroughputPerSecond, sinkApp.Get(0)->GetObject<PacketSink>(), filePlotThroughput.str (), f_num);
  }
  ...
  for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator i = stats.begin (); i != stats.end (); ++i)
    {
      ...
      std::cout << "  Throughput: " << data_mbytes * 8.0 * 1000000000 / NanoSeconds(i->second.timeLastRxPacket-i->second.timeFirstTxPacket) / 1000 / 1000  << " Mbps\n";
  ...
  std::ofstream myfile;
  myfile.open (id + "_drop.txt", std::fstream::in | std::fstream::out | std::fstream::app);
