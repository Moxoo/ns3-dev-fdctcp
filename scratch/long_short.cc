...
NS_LOG_COMPONENT_DEFINE ("long-short");
...
int main (int argc, char *argv[])
{
  ...
  std::string prefix_file_name = "long-short";
  uint32_t SNum = 5;
  std::string transport_port = "TcpDctcp";
  std::string queue_disc_type = "RedQueueDisc";//only for dctcp

  std::string queue_limit = "600p";
  double K = 100;//100 for 600buffer/65 for 250buffer
...
//  Ptr<UniformRandomVariable> random = CreateObject<UniformRandomVariable> ();
  ...
  // To enable logging
  LogComponentEnable("long-short", LOG_LEVEL_INFO);
...
//  LogComponentEnable("TcpDctcp", LOG_LEVEL_INFO);
  ...
  // Connect switch0 with switch1
  NetDeviceContainer devices;
  devices = neckLink.Install(switches.Get(0), switches.Get(1));
  ...
  for(uint32_t i = 0; i<receiver.GetN() ; i++)
  {
	  NetDeviceContainer devices;
	  devices = ptpLink.Install(switches.Get(1), receiver.Get(i));
	  address.NewNetwork();
	  Ipv4InterfaceContainer interfaces = address.Assign(devices);
...
//	  sourceApp.Stop(Seconds(stop_time));
  ...
  // short flows
  for(uint16_t i = 3; i<senders.GetN() ; i++)
  {
	  BulkSendHelper ftp("ns3::TcpSocketFactory", InetSocketAddress(sink_interfaces.GetAddress(1,0),port));
	  ftp.SetAttribute("SendSize", UintegerValue(TCP_SEGMENT));
	  ftp.SetAttribute("MaxBytes", UintegerValue(data_mbytes));
	  ApplicationContainer sourceApp = ftp.Install(senders.Get(i));
	  sourceApp.Start(Seconds(start_time_short));
	  sourceApp.Stop(Seconds(stop_time));
  }
  ApplicationContainer shortSinkApp = sinkHelper.Install(receiver.Get(1));
