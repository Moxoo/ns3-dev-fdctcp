...
{
  NS_LOG_FUNCTION (this);
  NS_LOG_DEBUG ("Enter Mark() ");
  if (!m_headerAdded && m_header.GetEcn () != Ipv4Header::ECN_NotECT)
    {
      m_header.SetEcn (Ipv4Header::ECN_CE);
//      std::cout<< "mark success" <<std::endl;
//      std::cout<< "**************" <<std::endl;
      return true;
    }
//  std::cout<< "mark failed" <<std::endl;
  return false;
}

//mushu code
bool
Ipv4QueueDiscItem::BeMarked (void)
{
  NS_LOG_FUNCTION (this);
  NS_LOG_DEBUG ("Has been marked or not");
  if(m_header.GetEcn() == Ipv4Header::ECN_CE)
  {
//	  std::cout<< "The chosen one has been marked" <<std::endl;
	  return true;
  }
//  std::cout<< "The chosen one has not been marked" <<std::endl;
  return false;
}
